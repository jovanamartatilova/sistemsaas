...
useEffect(() => {
  ...
  const fetchCompany = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8000/api"}/c/${idCompanyFromUrl}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:8000/api"}/auth/forgot-password-candidate`, {
        method: "POST",
        headers: {
