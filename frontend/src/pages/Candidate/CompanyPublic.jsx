...
const Icon = {
  location: (<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>),
  ...
  briefcase: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2" /></svg>),
  ...
  award: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="6" /><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11" /></svg>),
  ...
  handshake: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20.42 4.58a5.4 5.4 0 0 0-7.65 0l-.77.78-.77-.78a5.4 5.4 0 0 0-7.65 0C1.46 6.7 1.33 10.28 4 13l8 8 8-8c2.67-2.72 2.54-6.3.42-8.42z" /></svg>),
...
function AnimatedCounter({ value, duration = 1200 }) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => { start += 1; setCount(start); if (start >= end) clearInterval(timer); }, stepTime);
...
function StatCard({ icon, label, value, sub, color, barColors }) {
  return (
    <div style={{ background: "#fff", borderRadius: "16px", padding: "22px 24px", boxShadow: "0 1px 4px rgba(0,0,0,0.06), 0 4px 16px rgba(0,0,0,0.04)", display: "flex", flexDirection: "column", gap: "4px", alignItems: "center", textAlign: "center" }}>
      <div style={{ width: "38px", height: "38px", borderRadius: "10px", background: `${color}15`, display: "flex", alignItems: "center", justifyContent: "center", color }}>{icon}</div>
      <div style={{ fontSize: "28px", fontWeight: "800", color: "#1e293b", letterSpacing: "-1px", marginTop: "10px" }}><AnimatedCounter value={value} /></div>
      ...
      {barColors && (
        <div style={{ display: "flex", gap: "3px", marginTop: "10px", alignItems: "flex-end", height: "26px", width: "100%" }}>
          {barColors.map((c, i) => <div key={i} style={{ flex: 1, background: c, borderRadius: "3px 3px 0 0", height: `${28 + Math.sin(i * 1.4) * 18}%`, opacity: 0.45, minHeight: "4px" }} />)}
        </div>
      )}
...
function Chip({ label, color }) {
  return <span style={{ fontSize: 11, fontWeight: 700, padding: "5px 12px", borderRadius: 8, background: `${color}12`, color, border: `1px solid ${color}25`, textTransform: "capitalize" }}>{label}</span>;
}
  ...
  return (
    <div onClick={() => onClick(position)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ padding: "18px 20px 20px", display: "flex", flexDirection: "column", gap: 6, flex: 1 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: "#2d7ff3", background: "#eff6ff", padding: "2px 8px", borderRadius: 6, alignSelf: "flex-start", marginTop: 2 }}>{position.pivot?.quota || 0} Quota</span>
        <div style={{ flex: 1 }} />
        <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 5, fontSize: 11.5, color: "#94a3b8" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}><span style={{ color: "#94a3b8" }}>{Icon.calendar}</span>{formatDate(vacancy.start_date || vacancy.deadline)} – {formatDate(vacancy.end_date || vacancy.deadline)}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#ef4444", fontWeight: 600 }}><span style={{ color: "#ef4444" }}>{Icon.clock}</span>Closes: {formatDate(vacancy.deadline)}</div>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ background: "#fff", borderRadius: 24, width: "100%", maxWidth: 560, maxHeight: "90vh", overflowY: "auto", position: "relative", boxShadow: "0 25px 60px rgba(0,0,0,0.18)", border: "1px solid #e2e8f0" }}>
        ...
        <div style={{ padding: "28px 32px 32px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24, padding: 18, background: "#f8fafc", borderRadius: 14, border: "1px solid #e2e8f0" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, color: "#475569" }}><span style={{ color: "#2d7ff3" }}>{Icon.location}</span>{vacancy.location || "Jakarta"}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, color: "#475569" }}><span style={{ color: "#2d7ff3" }}>{Icon.calendar}</span>{formatDate(vacancy.start_date || vacancy.deadline)} – {formatDate(vacancy.end_date || vacancy.deadline)}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, color: "#ef4444", fontWeight: 600 }}><span style={{ color: "#ef4444" }}>{Icon.clock}</span>Registration Deadline: {formatDate(vacancy.deadline)}</div>
          </div>
...
function DropItem({ icon, label, color, onClick }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ width: "100%", padding: "9px 12px", background: hov ? (color ? "#fff1f2" : "#f8fafc") : "none", border: "none", borderRadius: 8, color: color || "#334155", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, textAlign: "left", fontFamily: "inherit", transition: "0.15s" }}>
      {icon} {label}
    </button>
  );
}
...
export default function CompanyPublicPage() {
  ...
  // ref untuk section Open Positions — target scroll
  ...
  const filteredPositions = searchQuery.trim() === ""
    ? positions
    : (() => {
        const keywords = searchQuery.toLowerCase().split(/\s+/).filter(Boolean);
        return positions.filter(pos => {
          const haystack = [pos.name, pos.vacancy?.title, pos.vacancy?.description, pos.vacancy?.location, pos.vacancy?.type, pos.vacancy?.payment_type, `batch ${pos.vacancy?.batch}`].join(" ").toLowerCase();
  ...
  // fungsi scroll — pakai anchor tag trick supaya pasti jalan
  ...
  if (loading) return <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center" }}><p style={{ color: "#94a3b8", fontSize: 14, fontFamily: "'Poppins',sans-serif" }}>Loading...</p></div>;
  if (!company) return <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center" }}><p style={{ color: "#94a3b8", fontSize: 14, fontFamily: "'Poppins',sans-serif" }}>Company Not Found</p></div>;
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "'Poppins','Segoe UI',sans-serif", color: "#0f172a" }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .hero-search { transition: border-color 0.2s, box-shadow 0.2s; }
        .hero-search:focus { outline: none !important; border-color: rgba(56,189,248,0.7) !important; box-shadow: 0 0 0 3px rgba(56,189,248,0.25) !important; }
        .hero-search::placeholder { color: rgba(255,255,255,0.4); }
      `}</style>

      {/* ── Navbar ─────────────────────────────────────────────── */}
      <header style={{ position: "sticky", top: 0, zIndex: 100, height: 60, padding: "0 64px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,0.95)", backdropFilter: "blur(16px)", borderBottom: "1px solid #e2e8f0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {isAuthenticated ? (
            <div ref={profileRef} style={{ position: "relative" }}>
              <button onClick={() => setProfileOpen(o => !o)} style={{ width: 36, height: 36, borderRadius: "50%", cursor: "pointer", background: profileOpen ? "#eff6ff" : "#f8fafc", border: "1.5px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", color: "#64748b" }}>{Icon.user}</button>
              {profileOpen && (
                <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, boxShadow: "0 12px 32px rgba(0,0,0,0.12)", minWidth: 200, padding: 6, zIndex: 200 }}>
                  <div style={{ padding: "10px 14px 8px", borderBottom: "1px solid #f1f5f9", marginBottom: "6px" }}>
      ...
      <main>
        {/* ── Hero ─────────────────────────────────────────────── */}
        <section style={{ background: "linear-gradient(160deg,#0f172a 0%,#1e3a5f 55%,#0f2744 100%)", padding: "60px 64px 52px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 1 }} className="fadein">
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
              ...
              <h1 style={{ fontSize: 48, fontWeight: 900, color: "#fff", margin: "0 0 12px", letterSpacing: "-2px", lineHeight: 1.08 }}>{company.name}</h1>

              <p style={{ fontSize: 15, color: "rgba(255,255,255,0.6)", lineHeight: 1.75, margin: "0 0 22px", maxWidth: 580 }}>
                {company.description || `${company.name} opens internship opportunities for students and fresh graduates to gain real-world experience, develop professional skills, and grow together with our team.`}
              </p>

              {/* Info Badges */}
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 24, justifyContent: "center" }}>
                {company.address && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "rgba(255,255,255,0.65)", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", padding: "7px 14px", borderRadius: 10 }}>
                    <span style={{ color: "#38bdf8" }}>{Icon.location}</span>{company.address}
                  </div>
                )}
                {company.email && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "rgba(255,255,255,0.65)", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", padding: "7px 14px", borderRadius: 10 }}>
                    <span style={{ color: "#38bdf8" }}>{Icon.mail}</span>{company.email}
                  </div>
                )}
              </div>
              ...
              <div style={{ width: "100%", maxWidth: 540, position: "relative" }}>
                <div style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,0.4)", pointerEvents: "none", display: "flex", alignItems: "center" }}>
                  {Icon.search}
                </div>
                <input
                  className="hero-search"
                  type="text"
                  placeholder="Search position, program, location... then press Enter"
                  value={searchQuery}
                  ...
                  style={{ width: "100%", padding: "13px 48px 13px 48px", borderRadius: 14, border: "1.5px solid rgba(255,255,255,0.2)", fontSize: 13.5, color: "#fff", fontFamily: "inherit", background: "rgba(255,255,255,0.12)", backdropFilter: "blur(12px)", boxShadow: "0 4px 20px rgba(0,0,0,0.2)" }}
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery("")} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.15)", border: "none", borderRadius: "50%", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "rgba(255,255,255,0.8)", padding: 0 }}>
                    {Icon.close}
                  </button>
                )}
              </div>
            ...
            <div style={{ display: "flex", justifyContent: "center", marginTop: 28 }}>
              <div style={{ width: "100%", maxWidth: 900, display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 16 }}>
                <div style={{ marginBottom: 4, width: "100%", textAlign: "center" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: "#38bdf8", background: "rgba(56,189,248,0.15)", border: "1px solid rgba(56,189,248,0.25)", padding: "5px 16px", borderRadius: 20, letterSpacing: "0.1em", textTransform: "uppercase", display: "inline-block" }}>Internship Program</span>
                </div>
                {[
                  ...
                  <div key={i} style={{ flex: "1 1 250px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 14, padding: "16px 18px", display: "flex", gap: 14, alignItems: "flex-start", animation: `fadeUp 0.5s ease ${0.1 + i * 0.12}s both` }}>
        ...
        <section style={{ padding: "60px 64px", maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
            <StatCard icon={Icon.briefcase} label="Active Programs" value={vacancies.length} sub="Open internship programs" color="#2d7ff3" barColors={["#3b82f6","#60a5fa","#93c5fd","#3b82f6","#60a5fa","#93c5fd","#3b82f6"]} />
            <StatCard icon={Icon.users} label="Open Positions" value={positions.length} sub="Roles available to apply" color="#10b981" barColors={["#4ade80","#86efac","#4ade80","#86efac","#4ade80","#bbf7d0","#4ade80"]} />
            <StatCard icon={Icon.star} label="Total Quota" value={totalQuota} sub="Spots for new interns" color="#f59e0b" barColors={["#fb923c","#fdba74","#fb923c","#fdba74","#fb923c","#fed7aa","#fb923c"]} />
          </div>
        </section>
        ...
        <section style={{ background: "#fff", padding: "70px 64px", borderTop: "1px solid #f1f5f9", borderBottom: "1px solid #f1f5f9" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: 52 }}>
              ...
              <p style={{ fontSize: 14, color: "#64748b", margin: "0 auto", maxWidth: 460, lineHeight: 1.7 }}>Gain the skills, experience, and connections you need to launch a successful career.</p>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 20 }}>
              {[
                ...
                <div key={i} style={{ padding: "28px 24px", borderRadius: 16, border: "1.5px solid #f1f5f9", background: "#fafbfc", transition: "0.2s" }}
                  ...
                  <div style={{ width: 48, height: 48, borderRadius: 12, background: `${item.color}12`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16, color: item.color }}>{item.icon}</div>
        ...
        {/* ── Open Positions ─────────────────────────────────────
            ref dipasang di sini sebagai target scroll dari search bar
        ─────────────────────────────────────────────────────────── */}
        <section ref={positionsSectionRef} style={{ padding: "70px 64px" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32, textAlign: "center", flexDirection: "column" }}>
              <div style={{ textAlign: "center", width: "100%" }}>
                ...
                <p style={{ fontSize: 13, color: "#94a3b8", margin: 0 }}>
                  {searchQuery
                    ? `${filteredPositions.length} result${filteredPositions.length !== 1 ? "s" : ""} for "${searchQuery}"`
            ...
            {filteredPositions.length > 0 ? (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(270px,1fr))", gap: 18 }}>
                {filteredPositions.map(pos => <PositionCard key={pos.id_position} position={pos} onClick={setSelectedPosition} />)}
              </div>
            ) : (
              <div style={{ textAlign: "center", padding: "64px 24px", background: "#fff", borderRadius: 16, border: "1.5px dashed #e2e8f0" }}>
                <div style={{ width: 56, height: 56, borderRadius: 16, background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", color: "#94a3b8" }}>{Icon.briefcase}</div>
                {searchQuery ? (
                  <>
                    <p style={{ fontSize: 15, fontWeight: 700, color: "#334155", margin: "0 0 6px" }}>No Results Found</p>
                    <p style={{ fontSize: 13, color: "#94a3b8", margin: "0 0 16px" }}>No positions match <strong>"{searchQuery}"</strong>. Try different keywords.</p>
                    <button onClick={() => setSearchQuery("")} style={{ padding: "8px 20px", borderRadius: 8, border: "1.5px solid #e2e8f0", background: "#fff", fontSize: 13, fontWeight: 600, color: "#64748b", cursor: "pointer", fontFamily: "inherit" }}>Clear Search</button>
                  </>
                ) : (
                  <>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div style={{ borderRadius: 24, background: "linear-gradient(135deg,#0f172a 0%,#1e3a5f 60%,#0f2744 100%)", padding: "60px 64px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 32, position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ position: "relative", zIndex: 1 }}>
                ...
                <p style={{ fontSize: 14, color: "rgba(255,255,255,0.55)", margin: 0, lineHeight: 1.7, maxWidth: 460 }}>Join {company.name}'s internship program and take the first step toward becoming a skilled professional.</p>
              </div>
              {!isAuthenticated ? (
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "60px 64px 32px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.5fr 2fr", gap: 64, marginBottom: 48 }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", lineHeight: 1.75, margin: "0 0 24px", maxWidth: 280 }}>{(company.description || `${company.name} opens opportunities for students and fresh graduates to grow with us.`).slice(0, 140)}...</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {company.address && <div style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 13, color: "rgba(255,255,255,0.7)" }}><span style={{ color: "#38bdf8", marginTop: 1, flexShrink: 0 }}>{Icon.location}</span>{company.address}</div>}
                  {company.email && <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "rgba(255,255,255,0.7)" }}><span style={{ color: "#38bdf8", flexShrink: 0 }}>{Icon.mail}</span>{company.email}</div>}
                </div>
              </div>
              <div>
                ...
                {emailSent ? (
                  <div style={{ padding: "20px", background: "rgba(56,189,248,0.1)", border: "1px solid rgba(56,189,248,0.3)", borderRadius: 14, fontSize: 14, color: "#38bdf8", fontWeight: 600, textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                    <span style={{ color: "#38bdf8" }}>{Icon.check}</span>Message sent successfully!
                  </div>
                ) : (
                  <form onSubmit={handleEmailSubmit} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <input className="input-field" placeholder="Your Name" value={emailForm.name} onChange={e => setEmailForm(f => ({ ...f, name: e.target.value }))} required style={{ padding: "11px 14px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.15)", fontSize: 13, color: "#fff", fontFamily: "inherit", background: "rgba(255,255,255,0.08)" }} />
                    <input className="input-field" type="email" placeholder="Your Email" value={emailForm.email} onChange={e => setEmailForm(f => ({ ...f, email: e.target.value }))} required style={{ padding: "11px 14px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.15)", fontSize: 13, color: "#fff", fontFamily: "inherit", background: "rgba(255,255,255,0.08)" }} />
                    <textarea className="input-field" placeholder="Your message..." value={emailForm.message} onChange={e => setEmailForm(f => ({ ...f, message: e.target.value }))} required rows={4} style={{ padding: "11px 14px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.15)", fontSize: 13, color: "#fff", fontFamily: "inherit", resize: "none", gridColumn: "1 / -1", background: "rgba(255,255,255,0.08)" }} />
                    <button type="submit" style={{ gridColumn: "1 / -1", padding: "12px 16px", borderRadius: 10, background: "linear-gradient(135deg,#2d7ff3,#38bdf8)", border: "none", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                      <span style={{ color: "#fff" }}>{Icon.send}</span>Send Message
                    </button>
                  </form>
                )}
              </div>
            ...
            <div style={{ paddingTop: 24, borderTop: "1px solid rgba(255,255,255,0.1)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <p style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", margin: 0 }}>Powered by <span style={{ color: "#38bdf8", fontWeight: 600 }}>EarlyPath</span></p>
            </div>
          </div>
        </footer>
      </main>
