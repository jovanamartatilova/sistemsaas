...
export default function RegisterApplicant() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Optional validation removed since we are just mocking
    ...
    try {
      ...
      const mockCompany = {
        id: 999,
        name: form.name || "Guest Applicant",
        role: "applicant"
      };
      ...
      setSuccess(true);
      setTimeout(() => navigate("/applicant/portal"), 1500);
    } catch (err) {
    ...
    <div className="min-h-screen w-full flex">
      ...
      >
        ...
        <div className="relative z-10 flex flex-col justify-end p-12 pb-16 text-left">
          ...
          <h2 className="text-4xl font-bold text-white mb-4 leading-tight">Start Your Future Today</h2>
          <p className="text-lg text-white/60 max-w-sm">Join thousands of applicants finding their dream internships through EarlyPath.</p>
        </div>
      </div>
      ...
      >
        <div className="w-full max-w-md text-left">
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">Applicant Sign Up</h1>
            <p className="text-white/40">Create your account to start applying</p>
          </div>
          ...
          {success ? (
            <div className="bg-emerald-500/10 border border-emerald-500/30 p-6 rounded-2xl text-center">
              ...
              <h3 className="text-white font-bold text-lg mb-2">Registration Success!</h3>
              <p className="text-white/60 text-sm">Redirecting to portal...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              >
                {loading ? "Signing up..." : "Create Applicant Account"}
              </button>

              <p className="text-center text-white/40 text-sm mt-6">
                Already have an account? <Link to="/applicant/login" className="text-blue-400 font-bold hover:underline">Login here</Link>
              </p>
            </form>
          )}
        </div>
