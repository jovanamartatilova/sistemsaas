  ...
  return (
    <button
      ...
      style={{
        ...
        padding: '4px 8px', borderRadius: '6px', fontSize: '10.5px', fontWeight: '600',
        cursor: 'pointer', border: '1px solid #e2e8f0',
        background: hov ? '#f1f5f9' : '#fff', color: '#475569',
...
export default function SelectionHR() {
  ...
  const [activeTab, setActiveTab] = useState(null); // final, or stage type (screening, test, interview)
  ...
  // Merged stages for visualizer (groups consecutive same-type stages)
  ...
  // Group stages by type for Tabs (ordered by first appearance in flow)
  const stageGroups = useMemo(() => {
    const groups = {};
    const order = [];
    selectionFlow.forEach((stage, i) => {
      if (!groups[stage.type]) {
        groups[stage.type] = [];
        order.push(stage.type);
      }
  ...
  // Reset tab when changing position or when stage groups change
  useEffect(() => {
  ...
  const handleTabChange = (tab) => {
    ...
    if (tab !== 'final') {
      ...
      if (group && group.stages.length > 0) {
        setActiveSubStageIndex(group.stages[0].globalIndex);
      }
    }
  };
  ...
  const fetchCandidates = (isSearch = false) => {
    ...
    // Determine target status filter
    if (activeTab === 'final') {
  ...
  useEffect(() => {
    if (activePositionId && activeTab) {
      if (activeTab === 'final' || activeSubStageIndex !== null) {
        fetchCandidates();
      }
    }
                ...
                candidates.map((c, i) => (
                  <div key={c.id_submission} style={{
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', justifyContent: 'flex-start' }}>
                      {c.has_cv && <DocBtn label="CV" onClick={() => viewDoc(c, 'cv')} />}
                      {c.has_portfolio && <DocBtn label="Porto" onClick={() => viewDoc(c, 'portfolio')} />}
                      {!c.has_cv && !c.has_portfolio && <span style={{ fontSize: '12px', color: '#cbd5e1' }}>No docs</span>}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-start' }}>
                      {activeTab !== 'final' && (
                        <>
                          <ActionBtn label={activeSubStageIndex === selectionFlow.length - 1 ? "Accept" : "Pass Next"} variant="green" onClick={() => handlePass(c)} />
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,22,40,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '28px', width: '360px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowLogoutModal(false)} style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid #e2e8f0', background: '#fff', cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleLogout} style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#ef4444', color: '#fff', cursor: 'pointer' }}>Sign Out</button>
            </div>
          </div>
        </div>
      )}
