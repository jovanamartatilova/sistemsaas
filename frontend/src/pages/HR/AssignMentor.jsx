...
export default function AssignMentorHR() {
  const [data, setData]         = useState({ stats: {}, interns: [], mentors: [] });
  const [loading, setLoading]   = useState(true);
  const [draft, setDraft]       = useState({}); // { id_submission: id_mentor } — pilihan sebelum confirm
  const [saved, setSaved]       = useState({}); // { id_submission: true }
  const [modal, setModal]       = useState(null); // intern object untuk confirm modal
  ...
  const fetchData = () => {
    api('/hr/assign-mentor')
      .then(res => {
        setData(res.data);
        // Init draft & saved dari data yang sudah ada di DB
        const initDraft = {};
        const initSaved = {};
  ...
  const mentorLoad = (mentorId) =>
    data.interns.filter(i => (draft[i.id_submission] || i.mentor_id) === mentorId).length;
